...
SAFE_METHODS_ALLOWED_FOR_USER_UPDATE = ['GET', 'POST', 'PUT', '', 'HEAD', 'OPTIONS']


class IsOwnerAndUpdateOnly(BasePermission):
    def has_permission(self, request, view):
        if request.method in SAFE_METHODS_ALLOWED_FOR_USER_UPDATE:
            return True
        return False
